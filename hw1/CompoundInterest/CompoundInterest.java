...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        return presentValue * Math.pow(1 + rate / 100, targetYear - (double)THIS_YEAR);
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return presentValue * Math.pow(1 + (rate - inflationRate) / 100, targetYear - (double)THIS_YEAR);
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double factor = 0.0;
        for(int i = THIS_YEAR; i <= targetYear; i++) {
            factor *= 1 + (rate - inflationRate) / 100;
            factor += 1;
        }
        return perYear * factor;
    }
